...
`idle_view`/`camera`, `iron_view`, `thirdperson_hand`, `lefthand_pos`, and `righthand_pos` receive the same generic treatment described below. Magazine, slide, bolt, trigger, and hand branches are ordinary animated parts. Native Bedrock ammunition visibility follows HMG's current round count for TaCZ's standard `bullet_in_barrel`, `bullet_in_mag`, and `bullet_chain` bones. A nonstandard ammunition bone is also recognized when every authored scale key for it in `static_bolt_caught` is zero; this covers rigs such as the RPG-7's `rocket` without a weapon-name exception. During a reload action, that action's authored scale keys retain control of the recognized bone. The loader does not infer attachment selection, shell/muzzle gameplay, or any other TaCZ state machine.
...
HMG's presentation frame is separate from imported model space. A single outer X half-turn maps the TaCZ model frame into the legacy HMG display basis at root traversal. Equipped first/third person, dropped items, inventory/GUI and nested gun traversal use that same boundary; child bones never apply it again. The exported-Bedrock boundary also applies TaCZ's 24-pixel root Y origin, Y-down cube coordinates, box/per-face UV order, mirror winding, and separate rest-plus-animation rotations. Native `.bbmodel` projects keep their existing transform conversion. The `3/16` import units are unaffected. Equipped Blockbench models additionally use TaCZ positioning nodes when present: first person aligns `idle_view` and blends its rigid inverse transform toward `iron_view` for ADS. Player-held native Bedrock rendering reverses the 1.7 `RenderPlayer` BLOCK_3D, Forge EQUIPPED_BLOCK and HMG half-scale transforms before aligning `thirdperson_hand` with the palm at `[-1,10,0]/16` relative to the right shoulder. Arm pose remains upstream; the native muzzle (-Z) points down the arm (+Y). `.bbmodel` retains its existing HMG hand-origin bridge. These paths bypass legacy `ModelEquipped`, `ModelHigh`, `SimpleADSOffset*`, `SprintingPoint`, `SprintingRotation` and configured third-person model placement; those settings remain active for OBJ/MQO guns. If `iron_view` is absent, ADS holds the idle view. `camera` is the idle fallback when `idle_view` is absent.

The third-person locator is a full inverse of its parent-local translation and ZYX rotation chain, not a world-space offset. Its translation uses the same units as geometry: TaCZ's `0.6` display factor scales both locator and mesh in TaCZ, whereas HMG's existing `ModelScala`/`InworldScale` scale both here. Applying `0.6` to the locator alone or flipping only its depth leaves a residual grip displacement. Native Bedrock GUI/creative previews keep the standard HMG inventory angle, authored fixed-origin translation and confirmed 1.2 context scale, with an additional eight GUI pixels to the right before model rotation/scaling. Other render contexts do not receive that offset.
...
| --- | --- | --- |
| `idle` | `static_idle` | Authored loop/hold; an omitted loop is treated as hold; zero-length pose holds |
...
Equipped imported guns use a dedicated `MOVEMENT` layer between static `BASE` pose and overriding `ACTION` clips. Ordinary fire remains the final additive layer. While any ACTION is active, movement playback is stopped and contributes no channels, including bones omitted by a sparse action clip. Starting, stopping, or completing movement therefore cannot replace an action transition snapshot. Locomotion selection itself changes presentation only; movement speed, sprint permission, stamina, and aiming authority remain HMG/Vanilla-owned.
...
`run_start` and `run_end` play once; moving/walking/holding clips loop. A transition clip is allowed to finish before the next movement state starts, so it is not restarted by every render pass. Imported movement clips use a short 0.12-second transition on entry, replacement, completion, and exit. That transition captures only the additive MOVEMENT contribution, so stopping a walk/run cannot replay or contaminate an ACTION pose. ADS selects the aiming walk and suppresses sprint presentation. Reload, cocking, and a held trigger also suppress sprint selection, while their existing action/fire layers retain precedence. Missing direction-specific clips fall back to forward walk; missing locomotion stops only the movement layer.

For held native Bedrock guns, pressing fire from the lowered sprint state reuses HMG's existing `set_up`/trigger state to leave the sprint pose immediately. The owning gun tick, on both logical sides, holds firing for four recovery ticks and retains a tapped trigger until the gun is ready. Reloading cancels that retained tap. This gate is scoped to `BedrockModel` presentation guns and does not change legacy OBJ/MQO, native `.bbmodel`, turret, or non-held-stack behavior; the server remains authoritative for the actual shot.
...
Legacy reload tracing (2026-09-15): the accepted server request sets `IsReloading` and zeroes `RloadTime`; its owning-client event is scheduled onto the client thread and initializes legacy presentation. Selected-stack `gunProcess` calls `proceedreload`, which advances `RloadTime` while reload/ammunition eligibility holds. Completion remains server-owned; per-shell interruption, unequip and cancellation retain their existing resets. Holding the reload key no longer zeroes the client clock on every repeated request. `RenderTickSmoothing` supplies the partial tick, and first-person rendering selects the live matching stack's `IsReloading` and `RloadTime` together even without an imported animation definition. The original `RloadTime + smoothing` calculation and final-frame snap feed `GunState.Reload`, `PartsRender_Gun.checkState2`, `getReloadmotion`, motion-key selection/interpolation and the legacy part transform. The pre-Bedrock interpolation and part traversal are retained. Without an imported definition, `AnimationClient.prepare`/`pose` cannot replace that legacy transform; the first-person base pose also retains the legacy reload branch. Static tests exercise the consumed NBT, advancing ticks, interpolation, key selection and resulting motion, but do not prove live packet timing, client eligibility or visible playback. Manual reload acceptance remains pending.
