...
public class HMGLivingUpdateEvent {
    ...
    public void onEntityHurt(LivingHurtEvent event) {
        if (event.entityLiving.worldObj.isRemote || event.source.isExplosion()
                || !(event.source instanceof handmadeguns.Util.HMGImpactDamageSource)) return;
        handmadeguns.Util.HMGImpactDamageSource source = (handmadeguns.Util.HMGImpactDamageSource) event.source;
        if (source.isHeadshot(event.entityLiving)) {
            event.ammount *= 1.75F;
            event.entityLiving.worldObj.playSoundAtEntity(event.entityLiving, "note.bd", 1.0F, 1.0F);
        }
    }
    @SubscribeEvent
